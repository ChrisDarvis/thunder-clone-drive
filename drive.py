...
try:
    bot = Client(
        ...
        api_id="3642426",
        api_hash="831f0f0ddd4b3dfb21dd7a3d1f2c3fd6",
        bot_token="1538333452:AAGa_PW1nDAArvDRz2h7tVOZQdhBZf0FZ0Y"
    )
    ...
    def handle_media(client, message):
        ...
        if gauth.credentials is None:
            message.reply_text(
                "Not Authenticated! Authenticate Google Drive using 'auth' command.",
                quote = True
            )
        else:
            ...
            driveFile = drive.CreateFile({
                'title': os.path.basename(file),         # message.caption
                'parents': [{
                    'kind': 'drive#fileLink',
                    'teamDriveId': '0AH0pUxFCGp6vUk9PVA',
                    'id': '1k0ly5oS0vJr_loe52bi2E1qLWl3jxG1f'
                }]
            })
